...
class Client {
    ...
    public function forceStandalone()
    {
        ...
        if($this->connected) {
            throw new Exception('Cannot force Client to use standalone PHP driver after a connection has already been established.');
        }
        ...
        if (preg_match('#^(tcp|unix)://(.*)$#', $this->host, $matches)) {
            if($matches[1] == 'tcp') {
                if ( ! preg_match('#^([^:]+)(:([0-9]+))?(/(.+))?$#', $matches[2], $matches)) {
                    throw new Exception('Invalid host format; expected tcp://host[:port][/persistence_identifier]');
                }
                $this->host = $matches[1];
    ...
    public function connect()
    {
        ...
        if ( ! $result) {
            ...
            throw new Exception("Connection to Redis {$this->host}:{$this->port} failed after $failures failures." . (isset($errno) && isset($errstr) ? "Last Error : ({$errno}) {$errstr}" : ""));
        }
    ...
    public function pSubscribe($patterns, $callback)
    {
        ...
        while ($patternCount--) {
            ...
            if ( ! $status) {
                throw new Exception('Invalid pSubscribe response.');
            }
        }
        try {
            while ($this->subscribed) {
                ...
                if ($type != 'pmessage') {
                    throw new Exception('Received non-pmessage reply.');
                }
                $callback($this, $pattern, $channel, $message);
            }
        } catch (Exception $e) {
            if ($e->getCode() == Exception::CODE_TIMED_OUT) {
                try {
    ...
    public function subscribe($channels, $callback)
    {
        ...
        while ($channelCount--) {
            ...
            $this->subscribed = $status > 0;
            if ( ! $status) {
                throw new Exception('Invalid subscribe response.');
            }
        }
        try {
            while ($this->subscribed) {
                ...
                if ($type != 'message') {
                    throw new Exception('Received non-message reply.');
                }
        ...
        else
        {
            ...
            // Wrap exceptions
                ...
                if ( ! ($result = $this->redis->IsConnected())) {
                    $this->connected = FALSE;
                    $code = Exception::CODE_DISCONNECTED;
                }
    ...
    protected function read_reply($name = '')
    {
        ...
        if($reply === FALSE) {
            ...
            if ($info['timed_out']) {
                throw new Exception('Read operation timed out.', Exception::CODE_TIMED_OUT);
            } else {
                $this->connected = FALSE;
        ...
        switch ($replyType) {
            ...
            case '-':
                if($this->isMulti || $this->usePipeline) {
                    ...
                    throw new Exception(substr($reply,0,4) == '-ERR' ? substr($reply, 5) : substr($reply,1));
                }
                break;
            /* Inline reply */
            case '+':
