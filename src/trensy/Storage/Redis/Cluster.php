...
class Cluster
{
    ...
    foreach ($servers as $server)
    {
      if(is_array($server)){
        ...
        $client = $server;
          ...
          throw new Exception('Server should either be an array or an instance of Client');
      }
      if($standAlone) {
  ...
  public function setMasterClient(Client $masterClient, $writeOnly=false)
  {
    if(!$masterClient instanceof Client){
        throw new Exception('Master client should be an instance of Client');
    }
  ...
  public function client($alias)
  {
    ...
    throw new Exception("Client $alias does not exist.");
  }
